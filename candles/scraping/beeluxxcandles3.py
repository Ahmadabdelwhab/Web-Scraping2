...
while page <= 6:
    try:
        ...
        result = requests.get(
            f"https://beeluxxcandles.com/collections/luxury-beeswax-candles?page={page}"
        )
        ...
        titles = soup.find_all("p", {"class": "h6"})
        price = soup.find_all("p", {"class": "h1 medium-down--left"})
        item_links = soup.find_all(
            "a", {"class": "product-grid-item product-list-item"}
        )
        main_url_images = soup.find_all(
            "div",
            {"class": "grid-item large--one-fifth medium--one-third"},
        )
...
with open(
    "C:/Users/Qebaa/OneDrive/Desktop/Web Scraping2/candles/beeswax candles3.csv",
    "w",
    newline="",
    encoding="utf-8",
) as myfile:
